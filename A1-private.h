...
struct A1_chip {
	...
	int atune_direction; /* 1 up, 0 down */
	int atune_level;
};
...
extern bool A1_wq_enqueue(struct work_queue *wq, struct work *work);
extern struct work *A1_wq_dequeue(struct work_queue *wq);
...
extern uint8_t *cmd_BIST_START(struct A1_chain *a1, uint8_t chip);
extern uint8_t *cmd_BIST_FIX(struct A1_chain *a1, uint8_t chip);
...
extern struct A1_extra_options A1_extra_options;
extern void A1_parse_options(void);
